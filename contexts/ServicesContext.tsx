...
export const ServicesProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    // Controla o carregamento inicial da aplicação.
    const initialLoad = async () => {
        setLoadingMessage('Conectando ao banco de ideias...');
        await fetchData();
        setIsLoading(false);
    };
    initialLoad();
  ...
  const refreshData = useCallback(async () => {
    // Controla as atualizações manuais, usando um estado separado.
    setIsRefreshing(true);
    setLoadingMessage('Sincronizando com a base de dados...'); // Mensagem específica para atualização
    await fetchData();
