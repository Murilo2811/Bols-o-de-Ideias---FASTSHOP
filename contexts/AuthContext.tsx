...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const handleAuthSuccess = (data: AuthResponse) => {
    // Garante que a role 'Leitor' seja atribuída por padrão se não vier da API
    const userWithRole = { ...data.user, role: data.user.role || 'Leitor' };
    setSession({ user: userWithRole, token: data.token });
    setError(null);
  };
